...
| Chip ignores a `secrets.h` that is clearly on disk | You built once *before* creating it. ESP-IDF compiles through ccache, which keys its cache on the headers the previous compile opened — `secrets.h` was not among them, so creating it invalidates nothing and the stale object comes back. `idf.py fullclean` does **not** help; the cache lives outside `build/`. Fix with one `CCACHE_RECACHE=1 idf.py build` (or `idf.py --no-ccache build`), after which normal builds are correct. The `config:` lines at boot are how you spot it — they print each value and where it came from |
